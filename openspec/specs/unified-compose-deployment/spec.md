...
Defines the repository-root Docker Compose surface as the sole canonical deployment definition for the project: a single `docker-compose.yml` at the repository root that brings up the STT service (`8001`), the matcher (`8002`), product identification (`8003`), the frontend, and Redis — individually or together — while preserving every application-level contract (build context, image, environment defaults, healthcheck, restart policy, least-privilege mounts).

Redis is a **soft** dependency of the matcher and carries no `depends_on`, which is what lets the "services are deliberately independent" promise survive its introduction: a dead Redis degrades the matcher to a Supabase read and never makes `POST /match` unavailable. The matcher's former read-only catalogue volume (`./data:/data:ro`) and its `CATALOGUE_DB` variable are gone — the catalogue now comes from Supabase, cached in Redis.
