...
> rev 5 (2026-07-25): redis-catalogue-cache — catalogue moved to Supabase + Redis snapshot cache; `catalogue_id` is now a `warehouses.code` (breaking); REQ-API-5 (read-only SQLite catalogue) removed — see `catalogue-source-supabase` and `catalogue-redis-cache` specs.

## Purpose

Defines the HTTP contract and deployment of the Module 3 matcher: a FastAPI service on port 8002 exposing `POST /match`, `GET /catalogues`, and `GET /health`, configured entirely via environment variables, reading its catalogue from Supabase (source of truth) through a Redis snapshot cache — see `catalogue-source-supabase` and `catalogue-redis-cache`.
