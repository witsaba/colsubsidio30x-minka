...
- [ ] 6.6 **NEW (orchestrator, 2026-07-25)** Fix REQ-AUD-2 gap: `GET /api/auditor/records` currently sends no theoretical stock, so the auditor's "Contado vs Sistema" comparison shows `—` for the system side even though `warehouse_stock_balances` has the real figure. Auditor-only (never operator-facing — RF-18 remains operator-only blindness, does NOT apply to the auditor route). RED test asserting the route joins `warehouse_stock_balances` (by `warehouse_id`+`product_id`) and returns `system_qty`/`system_unit_code` per record; GREEN wires it into `frontend/src/lib/auditor/records.ts` / `toAuditorRecord`, replacing the `"Sistema sin dato"` placeholder with the real value when available (may legitimately still be null for a record with no matching balance row — that's a real absence, not a wiring gap).
- [ ] 6.7 **NEW (orchestrator, 2026-07-25)** Fix REQ-AUD-4 gap: `auditor_actions` rows are written correctly but nothing reads them back, so the on-screen trace resets to `[]` on reload — "audit trail per record" (RF-32) does not survive a refresh even though the DB has the full history. RED test for a new `GET /api/auditor/records` (or a dedicated `auditor/actions` read, whichever fits the existing route shape better) returning each record's `auditor_actions` rows ordered by `created_at`; GREEN wires `AuditorReview.tsx`'s `trace` to seed from that data instead of `[]`.
