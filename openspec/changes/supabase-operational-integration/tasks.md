...
- [ ] 6.4 Manual end-to-end against the seeded plan: consent → plan select → dictate → anomaly → confirm → delete/redo → auditor approve → export CSV. Verify rows landed in `voice_consents`, `count_records`, `record_anomalies`, `auditor_actions`, `export_batches`, `export_lines` via MCP `execute_sql`. **BLOCKED** on `SUPABASE_SERVICE_ROLE_KEY` — not retrievable via MCP (Supabase's MCP tools deliberately only expose the publishable/anon key), needs the user to provide it. `SUPABASE_URL` = `https://blvdxsoaopcvtzawvgbt.supabase.co` (retrieved via MCP `get_project_url`, non-secret).
