...
PRODUCTION USAGE NOTE (2026-07-24, verify-report W1/W4 + A/B measurement):
ranking operates over RAW catalogue text — the measured 98.6% top-1
configuration. Only strip_accents (inside the rapidfuzz token_set processor),
trigrams, and trgm_similarity are on the production path. The Spanish
normalization helpers (normalize_for_match, strip_packaging, flip_gender,
pluralize_es, expand_abbrev) and trgm_word_similarity are promoted,
characterization-tested spike utilities RESERVED for future measured changes
(e.g. a max(raw, normalized) auto-accept experiment); wiring them into
ranking was measured WORSE (0.9837 vs 0.9860 top-1) and collapses 50 SKU
rows into identical strings. See Engram sdd/add-matching-service/w1-normalization-ab.
"""
